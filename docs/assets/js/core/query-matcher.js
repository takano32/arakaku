...
export class QueryMatcher {
  ...
  includes(values) {
    ...
    if (!q) return true;
    const searchString = values.filter(Boolean).join(" ").toLowerCase();
  ...
  eventMatches(event) {
    return this.includes([
      event.name, event.event_id, event.event_type, event.source_article_id, event.source_video_ids?.join(" "), event.inferred_from, event.inferred_confidence, event.summary,
  ...
  promotionMatches(promotion) {
    return this.includes([
      promotion.name, promotion.name_en, promotion.promotion_id, promotion.category, promotion.country_scope, promotion.summary, promotion.source_article_ids?.join(" "),
  ...
  mentionMatches(mention) {
    ...
    const d = repo.sourceDocumentById(mention.source_id);
    return this.includes([
      mention.mention_type, mention.entity_type, mention.entity_hint, mention.matched_text, mention.context, mention.source_id, mention.source_ref_id, mention.confidence, mention.notes,
      d?.title, d?.url,
    ]);
  }

  sourceDocumentMatches(document) {
    return this.includes([document.title, document.url, document.source_type, document.source_ref_id, document.content_preview, document.content_text, document.notes]);
  }
}
