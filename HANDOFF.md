...
### Phase 24: 名鑑の実データを折りたたみから可視位置へ (2026-06-15)

「名鑑のデータはすべて見やすいところに」という要件で、Phase 23 で詳細（折りたたみ `<details>`）に入れていた**実データ**を可視位置へ昇格した（ID・来歴メタは詳細のまま）。

- **公開選手カード**: 戦績ブロックに `出場 <tournament_entry_raw>` チップを追加。詳細から重複していた `名鑑勝率`（戦績行に勝率%表示済み）と `名鑑出場大会`（チップ化）を削除
- **管理「名鑑選手」カード**: 出場大会マーカーを可視チップ行に、キャッチコピーを可視 `<p>` に昇格（詳細からは除去。詳細は `numbers_fighter_id`/団体原文/メモ/出典のみ）
- **管理「名鑑記録」カード**: 試合形式 `bout_format` を可視メタ行へ、試合詳細 `detail_raw` を可視 `<p class="record-detail">`（`white-space:pre-line` で原文の改行保持）へ昇格（詳細からは除去）。`style.css` に `.record-detail` を追加
- 折りたたみに残るのは ID・原文・出典・行番号などの来歴/照合メタのみ。全 leaf フィールドの表示カバレッジ 100% は維持。viewer 限定変更（生成 JSON md5 不変）・`make check` green

### Phase 23: 名鑑データの全フィールド表示カバレッジ確保 (2026-06-14)

「名鑑にあるデータはすべて重要なのでビューのどこかで必ず見せる」という要件で、全 Numbers フィールドの表示カバレッジを監査し、欠落を解消した。
...
- **コメントのみ・挙動完全不変**を多重に検証: Python は 32 ファイルすべて HEAD と **AST 完全一致**（ロジック・docstring 不変）、生成 JSON は再ビルドで **md5 バイト一致**、JS は全ファイル `node --check` 通過、`make check` green。差分の削除は 2 行のみ（いずれもコメントの文言置換、コード削除ゼロ）
- マルチエージェント（10 バッチ × 付与＋正確性レビューの 2 段）で実施。レビュー段が不正確なコメント（先勝ち/後勝ち誤り、subscription の帰属、subset 判定 vs 完全一致 など多数）を訂正済み
- 方針: モジュールヘッダ＋要所インライン（行レベルの逐次解説はしない）。Python は `#` コメントのみ（docstring 非改変で AST 不変を担保）

これ以降にコードを変更するエージェントは、各ファイル冒頭のヘッダと `.agents/skills/*` ポインタから素早く文脈を掴める。
