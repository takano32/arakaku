...
- `source_document_bodies.json`（~557KB / 479 件）のうち通信タブが使うのは note_article 120 件分のみ。note 本文だけを別 JSON に分割すれば通信タブの遅延ロードを ~40% に縮小できる（ビルド + config + repository merge + テストが必要なので任意。Phase 17 でゲートは外したため体感の待ちは既に解消済み）
