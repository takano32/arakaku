...
- 選手や試合の情報が不足している場合、Apple Numbers 由来の名鑑データから自動的に補完され、「名鑑確認済み」バッジが表示されます。
- 選手カードには、名鑑由来の通算戦績（〇勝〇負）や実績マーカー（👑 🏆）を表示する統計ブロックが追加されます。
- データの突合・補完はすべてビューア上での実行時に行われ、ソースデータは純粋な事実を保ちます。
