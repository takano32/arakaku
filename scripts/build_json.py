...
def build_bouts() -> list[dict[str, Any]]:
    ...
    for r in read_csv(DATA_SRC/"bouts.csv"):
        ...
        out.append({"bout_id":r["bout_id"],"event_id":r["event_id"],"promotion_id":r["promotion_id"],"bout_order":int(r["bout_order"]) if none_if_empty(r.get("bout_order")) else None,"matchup":none_if_empty(r.get("matchup")) or f"{r.get('fighter_a','')} vs {r.get('fighter_b','')}".strip(),"division":none_if_empty(r.get("division")),"weight_class_id":none_if_empty(r.get("weight_class_id")),"bout_type":none_if_empty(r.get("bout_type")),"fighters":[{"fighter_id":fa,"name":r["fighter_a"],"corner":none_if_empty(r.get("fighter_a_corner")),"result":"win" if wi==fa or r.get("winner")==r.get("fighter_a") else "loss" if wi or none_if_empty(r.get("winner")) else "unknown"},{"fighter_id":fb,"name":r["fighter_b"],"corner":none_if_empty(r.get("fighter_b_corner")),"result":"win" if wi==fb or r.get("winner")==r.get("fighter_b") else "loss" if wi or none_if_empty(r.get("winner")) else "unknown"}],"winner_id":wi,"winner":none_if_empty(r.get("winner")),"loser_id":li,"loser":none_if_empty(r.get("loser")),"result_status":none_if_empty(r.get("result_status")) or ("known" if wi or none_if_empty(r.get("winner")) else "unknown"),"result":{"round":rv,"time":none_if_empty(r.get("time")),"method_raw":none_if_empty(r.get("method_raw")),"method_normalized":none_if_empty(r.get("method_normalized")),"technique":none_if_empty(r.get("technique")),"decision_score":none_if_empty(r.get("decision_score"))},"title":{"is_title_bout":bool_from_text(r.get("is_title_bout")) or False,"title_id":none_if_empty(r.get("title_id")),"title_result":none_if_empty(r.get("title_result")),"note":none_if_empty(r.get("title_note")) or ""},"source_article_id":none_if_empty(r.get("source_article_id")),"notes":none_if_empty(r.get("notes")) or "","inferred_from_video_id":none_if_empty(r.get("inferred_from_video_id")),"inferred_from_video_title":none_if_empty(r.get("inferred_from_video_title")),"inferred_confidence":none_if_empty(r.get("inferred_confidence"))})
    return out
def build_fighters() -> list[dict[str, Any]]:
    return [{"fighter_id":r["fighter_id"],"display_name":r["display_name"],"aliases":split_list(r.get("aliases")),"main_division":none_if_empty(r.get("main_division")),"main_promotion_id":none_if_empty(r.get("main_promotion_id")),"profile":{"height":none_if_empty(r.get("height")),"age":none_if_empty(r.get("age")),"gym":none_if_empty(r.get("gym"))},"summary":none_if_empty(r.get("summary")) or "","source_article_ids":split_list(r.get("source_article_ids")),"inferred_from_video_ids":split_list(r.get("inferred_from_video_ids")),"inferred_confidence":none_if_empty(r.get("inferred_confidence"))} for r in read_csv(DATA_SRC/"fighters.csv")]
def build_titles() -> list[dict[str, Any]]:
    g={}
    for r in read_csv(DATA_SRC/"titles.csv"):
        tid=r["title_id"]
        g.setdefault(tid,{"title_id":tid,"promotion_id":r["promotion_id"],"division":r["division"],"lineage":[]})["lineage"].append({"order":int(r["order"]) if none_if_empty(r.get("order")) else None,"fighter_id":none_if_empty(r.get("fighter_id")),"fighter_name":r["fighter_name"],"reign_label":none_if_empty(r.get("reign_label")),"won_at_event_id":none_if_empty(r.get("won_at_event_id")),"lost_at_event_id":none_if_empty(r.get("lost_at_event_id")),"source_article_id":none_if_empty(r.get("source_article_id")),"source_video_id":none_if_empty(r.get("source_video_id"))})
    ...
    return [
        {
            ...
            "url": row["url"],
            "title": row["title"],
            "original_title": none_if_empty(row.get("original_title")),
